...
protected:
	...
	typedef typename std::map<Vertex_handle, Halfedge_handle> VertexHalfedgeMap;
	typedef typename VertexHalfedgeMap::iterator VHIter;
	typedef typename std::pair<Vertex_handle, Halfedge_handle> VertexHalfedgePair;
	typedef typename std::map<Halfedge_handle, Halfedge_handle> HalfedgeHalfedgeMap;
	typedef typename HalfedgeHalfedgeMap::iterator HHIter;
	typedef typename std::pair<Halfedge_handle, Halfedge_handle> HalfedgeHalfedgePair;
	VertexHalfedgeMap vertex_save;
	HalfedgeHalfedgeMap nxt_edeg_save;
...
public:
	...
	{
		if (v1 < new_vertices)
			vertex_save.insert(VertexHalfedgePair(index_to_vertex_map[v1], index_to_vertex_map[v1]->halfedge()));
		if (v2 < new_vertices)
			vertex_save.insert(VertexHalfedgePair(index_to_vertex_map[v2], index_to_vertex_map[v2]->halfedge()));
		if (v3 < new_vertices)
			vertex_save.insert(VertexHalfedgePair(index_to_vertex_map[v3], index_to_vertex_map[v3]->halfedge()));

		VHIter iter = vertex_save.begin();
		while (iter != vertex_save.end())
		{
			Halfedge_handle end = (*iter).second;
			if (end != Halfedge_handle())
			{
				...
				do {
					if (e->is_border())
					{
						nxt_edeg_save.insert(HalfedgeHalfedgePair(e, e->next()));
					}
					Halfedge_handle nxt = e->next();
					if(nxt->is_border())
					{
						nxt_edeg_save.insert(HalfedgeHalfedgePair(nxt, nxt->next()));
					}
					e = nxt->opposite();
				} while (e != end);
			}
	...
	void restore(Facet_handle fh)
	{
		...
		while ( iter != vertex_save.end())
		{
			set_vertex_to_edge_map((*iter).first->id(), (*iter).second);
			++iter;
		}

		HHIter hiter = nxt_edeg_save.begin();
		while (hiter != nxt_edeg_save.end())
		{
			(*hiter).first->HBase::set_next((*hiter).second);
			decorator.set_prev((*hiter).second, (*hiter).first);
			if ((*hiter).first->face() == fh)
...
// OPERATIONS
    ...
    Vertex_handle add_vertex( const Point_3& p) {
        ...
        if ( hds.size_of_vertices() >= hds.capacity_of_vertices()) {
            Verbose_ostream verr( m_verbose, std::cout);
            verr << " " << std::endl;
