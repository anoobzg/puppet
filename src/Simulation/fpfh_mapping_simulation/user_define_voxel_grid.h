...
class UserVoxelGrid : public pcl::Filter<PointT>
{
protected:
	...
	typedef typename pcl::PointCloud<NormalT> NormalCloud;
	typedef typename NormalCloud::Ptr NormalCloudPtr;

	typedef boost::shared_ptr< UserVoxelGrid<PointT, NormalT> > Ptr;
	typedef boost::shared_ptr< const UserVoxelGrid<PointT, NormalT> > ConstPtr;
...
public:
	...
	/** \brief Destructor. */
	virtual ~UserVoxelGrid()
	{
	}

	void setOutNormalCloud(NormalCloudPtr normal_cloud)
	{
		out_normal_cloud = normal_cloud;
	}

	void setInNormalCloud(NormalCloudPtr normal_cloud)
	{
		in_normal_cloud = normal_cloud;
	}
...
protected:
	...
	NormalCloudPtr out_normal_cloud;
	NormalCloudPtr in_normal_cloud;
...
UserVoxelGrid<PointT, NormalT>::applyFilter(PointCloud &output)
{
	...
	out_normal_cloud->points.resize(total);
	...
	for (unsigned int cp = 0; cp < first_and_last_indices_vector.size(); ++cp)
	{
		...
		else
		{
			...
			pcl::CentroidPoint<NormalT> normald;
			// fill in the accumulator with leaf points
			for (unsigned int li = first_index; li < last_index; ++li)
			{
				centroid.add(input_->points[index_vector[li].cloud_point_index]);
				normald.add(in_normal_cloud->points[index_vector[li].cloud_point_index]);
			}
			centroid.get(output.points[index]);
			normald.get(out_normal_cloud->points[index]);

			NormalT& n = out_normal_cloud->points[index];
			float d = std::sqrtf(n.normal_x * n.normal_x + n.normal_y * n.normal_y + n.normal_z * n.normal_z);
			if (d > 0.0f)
			{
				n.normal_x /= d;
				n.normal_y /= d;
	...
	out_normal_cloud->width = static_cast<uint32_t>(out_normal_cloud->points.size());
}