...
bool Scene::OnKey(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)
{
	...
	else if (ea.getEventType() == osgGA::GUIEventAdapter::KEYDOWN && ea.getKey() == osgGA::GUIEventAdapter::KEY_W)
	{
		_resetCurvature();
	}
...
void Scene::ResetCamera()
{
	...
	SetNearFar(0.01f, 1000.0f);
}

void Scene::_resetCurvature()
{
	std::ifstream inFile("curvatureData.txt", std::ios::in);
	if (!inFile.is_open())
	{
		throw "FATAL ERROR.";
	}

	float* curvatureData = new float[m_mesh.vertex_number];
	float temp;
	unsigned i = 0;
	while (inFile >> temp)
	{
		curvatureData[i] = temp;
		i++;
	}

	m_surface->SetCurvatureData(curvatureData);
	osg::Geode* geode = CreateMeshGeode(m_mesh);
	m_render_node->RemoveAll();
	m_render_node->AddChild(geode);
	m_picker->SetNode(geode);
