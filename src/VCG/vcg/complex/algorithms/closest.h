...
namespace vcg {
	namespace tri {
		...
		/*
		aka MetroCore
		given me a mesh ug on its faces is the point m the near a given point.	
		*/

		// input: mesh, point grid (g), distance limit (mdist)
		// output: normal (interpolated) to the face and the most close to it, and coord center of gravity of the point found

		// Note that the parameter template GRID there should be, as it must be 
		// UGrid<MESH::FaceContainer>, but I could not define it implicitly
